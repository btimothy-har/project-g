...
class BotClashClient():
    ...
    def __init__(self,bot:Optional[Red]=None):
        ...
        if not self._is_initialized:
            self.thread_pool = ThreadPoolExecutor(max_workers=1*3)
            self.read_thread_pool = ThreadPoolExecutor(max_workers=3*3)
            self.write_thread_pool = ThreadPoolExecutor(max_workers=3*3)
