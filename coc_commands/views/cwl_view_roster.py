...
class CWLRosterDisplayMenu(MenuPaginator):
    ...
    async def start(self):
        if self.league_clan.status in ['CWL Started']:
            self.reference_list = sorted(self.league_clan.master_roster,key=lambda x:(x.player.town_hall.level,x.player.hero_strength),reverse=True)
        else:
            ...
            self.reference_list = sorted(self.league_clan.participants,key=lambda x:(x.player.town_hall.level,x.player.hero_strength),reverse=True)
            mem_in_clan = await asyncio.gather(*[aPlayer.create(tag)for tag in list(self.league_clan.clan.members_dict.keys())])
            async for mem in AsyncIter(mem_in_clan):
                if mem.tag not in [p.tag for p in self.reference_list]:
                    self.reference_list.append(WarLeaguePlayer(mem.tag,self.league_clan.season))
        self.is_active = True
    ...
    async def _set_roster_status_content(self):
        ...
        for i, members_chunk in enumerate(chunked_members):
            ...
            if self.league_clan.status in ["CWL Started"]:
                header_text += f"\n**Participants:** {len([p for p in self.league_clan.master_roster if p.player.clan.tag == self.league_clan.tag])} In Clan / {len([p for p in self.league_clan.master_roster])} in CWL"
            else:
                header_text += f"\n**Rostered:** {len([p for p in self.league_clan.participants if p.player.clan.tag == self.league_clan.tag])} In Clan / {len([p for p in self.league_clan.participants])} Rostered"
            ...
            member_text = "\n".join([
                ...
                + (f"{EmojisUI.LOGOUT}" if player.player.clan.tag != self.clan.tag else f"{EmojisUI.SPACER}")
    ...
    async def _set_roster_strength_content(self):
        ...
        for i, members_chunk in enumerate(chunked_members):
            ...
            header_text += f"\n**League:** {EmojisLeagues.get(self.league_clan.league)}{self.league_clan.league}"
            if self.league_clan.status in ["CWL Started"]:
                header_text += f"\n**Participants:** {len([p for p in self.league_clan.master_roster if p.player.clan.tag == self.clan.tag])} In Clan / {len([p for p in self.clan.master_roster])} in CWL"
                header_text += f"\n*Only showing players in the in-game master roster.*"
            else:
                header_text += f"\n**Rostered:** {len([p for p in self.clan.participants if p.player.clan.tag == self.clan.tag])} In Clan / {len([p for p in self.clan.participants])} Rostered"
            ...
            member_text = "\n".join([
                ...
                + f"`{'':<1}{getattr(player.player.get_hero('Barbarian King'),'level',''):>2}"
                + f"{'':<2}{getattr(player.player.get_hero('Archer Queen'),'level',''):>2}"
                + f"{'':<2}{getattr(player.player.get_hero('Grand Warden'),'level',''):>2}"
                + f"{'':<2}{getattr(player.player.get_hero('Royal Champion'),'level',''):>2}"
                + f"{'':<2}{re.sub('[_*/]','',player.name)[:15]:<15}`"
                for player in members_chunk]
                )                
            embed = await clash_embed(
                context=self.ctx,
                ...
                message=header_text+member_text+f"\n\n**{startend}**",
                thumbnail=self.clan.clan.badge,
                )
            embeds.append(embed)
        self.paginate_options = embeds
        self.page_index = 0
