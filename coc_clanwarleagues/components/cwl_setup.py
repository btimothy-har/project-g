...
class CWLSeasonSetup(DefaultView):
    ...
    async def _callback_clan_select(self,interaction:discord.Interaction,select:DiscordSelectMenu):
        ...
        currently_participating = await self.coc_client.get_league_clans(season=self.season,participating=True)
        ...
        async for clan_tag in AsyncIter(select.values):
            league_clan = await self.coc_client.get_league_clan(clan_tag,season=self.season)
    ...
    async def get_embed(self):
        current_signups = await self.coc_client.get_league_players(season=self.season,registered=True)
        ...
        participating_clans = await self.coc_client.get_league_clans(season=self.season,participating=True)
        async for cwl_clan in AsyncIter(participating_clans):
            league_group = await self.coc_client.get_league_group(clan_tag=cwl_clan.tag,season=self.season)
            embed.add_field(
                ...
                value=f"# in Roster: {len(cwl_clan.participants)} (Roster {'Open' if cwl_clan.roster_open else 'Finalized'})"
                    ...
                    + (f"\nMaster Roster: {len(cwl_clan.master_roster)}" if league_group else "")
                    + "\n\u200b",
                inline=False
                )
        return embed
    ...
    async def build_clan_selector(self):
        ...
        if not self.season.cwl_signup_lock:
            ...
            async for c in AsyncIter(clans):
                league = await self.coc_client.get_league_clan(clan_tag=c.tag,season=self.season)
