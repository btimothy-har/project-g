...
class LegendsTourney(commands.Cog,GlobalClient):
    ...
    async def leaderboard_current_season_embed(self):
        ...
        async for i,chunk in c_iter.enumerate(start=1):
            player_text = "\n".join([
                f"{p.town_hall.emoji} `{p.clean_name[:15]:<15} {p.trophies:<6,}` <@{p.discord_user}>" for p in chunk
                ])
            if i == 1:
    ...
    async def generate_event_export(self):
        ...
        async for player in a_iter:
            ...
            tourn_period = (check_window_end.int_timestamp - check_window_start.int_timestamp)/3600
