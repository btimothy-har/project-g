...
class BasicPlayer():
    ...
    def new_member(self,user_id:int,home_clan:BasicClan):
        ...
        self.is_member = True
        bot_client.cog.coc_data_log.info(f"Player {self} is now an Alliance member!")

    def remove_member(self):
        self.home_clan = aPlayerClan()
        self.is_member = False
        self.last_removed = pendulum.now()
        bot_client.cog.coc_data_log.info(f"Player {self} has been removed as a member.")

    @property
    def share_link(self) -> str:
        return f"https://link.clashofclans.com/en?action=OpenPlayerProfile&tag=%23{self.tag.strip('#')}"
    
    async def get_full_player(self):
        return await bot_client.cog.fetch_player(self.tag)
    ...
    def cached_xp_level(self,new_value:int):
        ...
        bot_client.cog.coc_data_log.debug(f"{self}: cached_xp_level changed to {new_value}.")
    ...
    def cached_townhall(self,new_value:int):
        ...
        bot_client.cog.coc_data_log.debug(f"{self}: cached_townhall changed to {new_value}.")
    ...
    def discord_user(self,new_value:int):
        ...
        bot_client.cog.coc_data_log.info(f"{self}: discord_user changed to {new_value}.")
    ...
    def is_member(self) -> bool:
        ...
        if val and not getattr(self.home_clan,'is_alliance_clan',False):
            self.remove_member()
            bot_client.cog.coc_data_log.info(f"{self}: Removed as Member as their previous Home Clan is no longer recognized as an Alliance clan.")
            return False
        return val
    @is_member.setter
    def is_member(self,new_value:bool):
        db_Player.objects(tag=self.tag).update_one(set__is_member=new_value,upsert=True)
        bot_client.cog.coc_data_log.info(f"{self}: is_member changed to {new_value}.")
    ...
    def home_clan(self,new_value:aPlayerClan):
        ...
        bot_client.cog.coc_data_log.info(f"{self}: home_clan changed to {new_value}.")
    ...
    def first_seen(self,new_value:pendulum.DateTime):
        ...
        bot_client.cog.coc_data_log.debug(f"{self}: first_seen changed to {new_value}.")
    ...
    def last_joined(self,new_value:pendulum.DateTime):
        ...
        bot_client.cog.coc_data_log.info(f"{self}: last_joined changed to {new_value}.")
    ...
    def last_removed(self,new_value:pendulum.DateTime):
        db_Player.objects(tag=self.tag).update_one(set__last_joined=new_value.int_timestamp,upsert=True)
        bot_client.cog.coc_data_log.info(f"{self}: last_removed changed to {new_value}.")
