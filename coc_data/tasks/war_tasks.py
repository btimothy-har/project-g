...
class DefaultWarTasks():
    ...
    async def sync_clan_war_leagues(war:bClanWar):
        if war.type == ClanWarType.CWL:
            ...
            league_group = await GlobalClient.coc_client.get_league_group(clan_tag=war.clan.tag)
            if league_group:
                await league_group.compute_group_results()
    ...
    async def check_war_role(war:bClanWar):
        ...
        iter = AsyncIter([war.clan,war.opponent])

        async for clan in iter:
...
#                 try:
#                     runtime = et - st
#                     self.run_time.append(runtime.in_seconds())
#                 except:
#                     pass
